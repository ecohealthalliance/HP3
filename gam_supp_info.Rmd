...
```{r fit-all-zoo-noprim, cache=TRUE}
data_set = hosts %>% 
  filter(hMarOTerr == "Terrestrial",
         hWildDomFAO == "wild",
         hOrder != "PRIMATES",
...
all_zoonoses_noprim = fit_all_gams(data_set,
                            outcome_variable,
                            poisson,
                            terms)
```
...
```{r all-zoo-noprim-table, cache=FALSE}
all_zoonoses_noprim %>%
  filter(daic <= 2) %>%
  select(daic, terms) %>% 
  mutate(terms = stri_replace_all_regex(terms, "s\\(([^\\)]*)\\)", "$1")) %>%
...
```{r all-zoo-noprim-plot, fig.width=10, fig.height=12, cache=FALSE}
plot(all_zoonoses_noprim$model[[1]], all.terms=TRUE, pages=1, scale=0, residuals = TRUE)
```

Summary of top model:

```{r all-zoo-noprim-summary, cache=FALSE}
summary(all_zoonoses_noprim$model[[1]])
```
...
```{r make-svg-plot, eval=FALSE, include=FALSE}
...
svglite(file="Figure04-viral-traits.svg", width = convertr::convert(183, "mm", "in"), convertr::convert(117, "mm", "in"), pointsize=7)
allplots# + cowplot::draw_grob(legend, x=0.1, y=0.1)
dev.off()
source('plot_best_gam_all_vir.R')
```
