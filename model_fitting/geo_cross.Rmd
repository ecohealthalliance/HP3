...
```{r all-zoo, cache=TRUE, out.width=1200, out.height=800}
data_set = hosts %>% 
...
results %>% arrange(fold) %>%
  ...
  kable(caption = "Biased Predictions Regions (n > 10)")

results %>% arrange(fold) %>%
  filter(n_validate > 10, p_value >= 0.05) %>%
  kable(caption = "Unbiased Prediction Regions (n > 10)")

```

# All Viruses GAM - All Associations

```{r all-viruses, cache = TRUE, out.width=1200, out.height=800}
###### ALL VIRUSES
...
```{r strict-zoo, cache = TRUE, out.width=1200, out.height=800}
...
```{r strict-viruses, cache = TRUE, out.width=1200, out.height=800}
