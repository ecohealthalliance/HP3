...
summary(allz_region_gam)


get_relative_contribs(allz_rep_gam) %>% kable(caption = "All Zoonoses: Original Model")
```

```{r kable-allz-contribs}
get_relative_contribs(allz_region_gam) %>% kable(caption = "All Zoonoses: Region Model")
...
Adding zoogeographical region as a random effect to our best-fit All Zoonoses GAM does not improve the model (AIC~region~: `r round(allz_region_gam$aic,3)` vs AIC~original~: `r round(allz_rep_gam$aic,3)`); the effective degrees of freedom for the region smooth is very low (`r round(summary(allz_region_gam)$edf[8],5)`), which supports removing the region variable from the All Zoonoses GAM.
...
summary(allv_region_gam)

get_relative_contribs(allv_rep_gam) %>% kable(caption = "All Viruses: Original Model")

get_relative_contribs(allv_region_gam) %>% kable(caption = "All Viruses: Region Model")


```
  
  
Adding zoogeographical region as a random effect to our best-fit All Viruses GAM significantly improves the model (AIC~region~: `r round(allv_region_gam$aic,3)` vs AIC~original~: `r round(allv_rep_gam$aic, 3)`). The smooth for region is significant, as seen in the model details above. The deviance explained by the original model was `r 1-round(allv_rep_gam$deviance/allv_rep_gam$null.deviance,3)`.
...
summary(sz_region_gam)

get_relative_contribs(sz_rep_gam) %>% kable(caption = "Strict Zoonoses: Original Model")

get_relative_contribs(sz_region_gam) %>% kable(caption = "Strict Zoonoses: Region Model")


```
  
  
Adding zoogeographical region as a random effect to our best-fit Strict Zoonoses GAM significantly improves the model (AIC~region~: `r round(sz_region_gam$aic,3)` vs AIC~original~: `r round(sz_rep_gam$aic, 3)`). The smooth for region is significant, as seen in the model details above. The deviance explained by the original model was `r 1-round(sz_rep_gam$deviance/sz_rep_gam$null.deviance,3)`.
...
summary(strictv_region_gam)

get_relative_contribs(strictv_rep_gam) %>% kable(caption = "Strict Viruses: Original Model")

get_relative_contribs(strictv_region_gam) %>% kable(caption = "Strict Viruses: Region Model")

```
  
  
Adding zoogeographical region as a random effect to our best-fit Strict Viruses GAM significantly improves the model (AIC~region~: `r round(strictv_region_gam$aic,3)` vs AIC~original~: `r round(strictv_rep_gam$aic, 3)`). The smooth for region is significant, as seen in the model details above. The deviance explained by the original model was `r 1-round(strictv_rep_gam$deviance/strictv_rep_gam$null.deviance,3)`.
