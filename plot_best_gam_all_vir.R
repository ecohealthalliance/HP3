...
partials <- as.data.frame(lapply(1:ncol(preds$fit), function(cl) {
  x = lfun(model_data[[gterms$response]]) - rowSums(preds$fit[,-cl]) - intercept
...
smooth_titles = c("Host Disease Citations (Log)", "Phylogenetically Corrected Mass (Log)", "Host Habitat Area (Log)", "Species with 20% Range Overlap")
...
smooth_plots = map(names(smooth_data), function(smooth_term) {
  ...
  #  geom_hline(yintercept = (intercept), size=0.5, col="red") +
    ...
    ggtitle(smooth_titles[smooth_term]) + 
    scale_y_continuous(limits=c(-2,2), oob=scales::rescale_none) +
...
smooth_plots[[3]] = smooth_plots[[3]] + ylab("")
smooth_plots[[4]] = smooth_plots[[4]] + ylab("Strength of Effect")
...
#  geom_hline(yintercept = (intercept), size=0.5, col="red") +
  ...
  scale_fill_manual(breaks = c(FALSE, TRUE), values=c("darkgreen")) +
  scale_x_continuous(breaks = bin_data$no, labels = bin_data$labels) +
  noamtools::theme_nr +
  scale_y_continuous(limits=c(-2,2), oob=scales::rescale_none, name="") +
