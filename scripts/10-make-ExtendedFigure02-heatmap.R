...
dev.off()
hm <- pheatmap(z2, col=c("white", col=brewer.pal(9, "YlOrRd")),
               ...
               fontsize_col = 12, fontsize_number=10)

library(grid)
pdf(P("figures", "ExtendedFigure02-heatmap.pdf"), width = 11, height = 8.5)
pushViewport(viewport(x=unit(0.5, "in"), y=unit(1.5, "in"), width = unit(10, "in"), height=unit(6.5, "in"), just = c("left", "bottom")))
grid.draw(hm$gtable)
dev.off()
