...
source(P("supplement/preprocess_data.R"))
```
...
```{r all-zoo-cv, cache=FALSE}
cv_gam(all_zoonoses$model[[1]]) %>%
  mutate(deviance_explained = paste0(signif(deviance_explained*100, digits=3), "%")) %>% 
  rename(Fold = fold, Observations=n, `Deviance Explained` = deviance_explained) %>%
  kable(align=c("l","r", "r"))
```
...
```{r all-zoo-strict-cv, cache=FALSE}
...
```{r all-zoo-norev-cv, cache=FALSE}
...
```{r all-vir-stict-cv, cache=FALSE}
...
```{r vtraits-strict-cv, cache=FALSE}
...
domestics = read_excel(P("data/HP3_domestic-categories.xlsx")) %>% 
...
```{r dom-vir-strict-cv, cache=FALSE}
...
saveRDS(vtraits$model[[1]], P("supplement/viral_traits_model.rds"))
saveRDS(viruses, P("supplement/virus_data_processed.rds"))
saveRDS(all_viruses$model[[1]], P("supplement/all_viruses_model.rds"))
saveRDS(all_zoonoses$model[[1]], P("supplement/all_zoonoses_model.rds"))
